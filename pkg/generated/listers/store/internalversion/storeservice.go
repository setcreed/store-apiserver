...
type StoreServiceLister interface {
	...
	List(selector labels.Selector) (ret []*store.StoreService, err error)
...
// List lists all StoreServices in the indexer.
func (s *storeServiceLister) List(selector labels.Selector) (ret []*store.StoreService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*store.StoreService))
	})
	return ret, err
}
...
type StoreServiceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*store.StoreService, error)
...
// List lists all StoreServices in the indexer for a given namespace.
func (s storeServiceNamespaceLister) List(selector labels.Selector) (ret []*store.StoreService, err error) {
...
// Get retrieves the StoreService from the indexer for a given namespace and name.
func (s storeServiceNamespaceLister) Get(name string) (*store.StoreService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(store.Resource("storeservice"), name)
	}
	return obj.(*store.StoreService), nil
}
